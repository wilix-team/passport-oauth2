...
OAuth2Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query.state && req.query.session_state) {
    callbackURL = callbackURL + '?' + Object.keys(req.query)
      .filter(function(key) { return ['state', 'session_state', 'code'].indexOf(key) === -1; })
      .map(function(key) { return key + '=' + req.query[key]; })
      .join('&');
    callbackURL = callbackURL[callbackURL.length - 1] === '?' ? callbackURL.substring(0, callbackURL.length - 1) : callbackURL;
  }

  var authorizationURL = options.authorizationURL || this._oauth2._authorizeUrl;
  ...
  if (req.query && req.query.code) {
    ...
    if (state && typeof state == 'string') {
      ...
      var parsed = url.parse(authorizationURL, true);
